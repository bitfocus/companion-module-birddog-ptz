...
export function getModelActionDetails(array, FW, model) {
	...
	let modelActions = array.filter(
		...
		(array) =>
			(array.camera.includes(model) || (array.camera.includes('All') && model != 'KBD')) &&
			(array.firmware?.includes(FW) ?? true),
	)
...
export function getModelQueries(array, model, FW) {
	...
	return Object.fromEntries(
		asArray.filter(
			(item) =>
				(item[1].camera.includes(model) || (item[1].camera.includes('All') && model != 'KBD')) &&
